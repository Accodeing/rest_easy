    ...
    module FromJSON
      ...
      def convert_collection( key, collection )
        ...
        if Registry.key?(mapper_name)
          ...
          raise MissingModelOrMapperException, "for #{ key } (#{ mapper_name }, #{ RestEasy::Mapper::DefaultTemplates.canonical_name_sym }) with #{ collection }" if ENV['RUBY_ENV']
          RestEasy.logger.warn( "Missing Model or Mapper implementation for #{ key } with attributes: #{ collection }" )
